...
- Added **multi-platform packaging**: `electron-builder` targets Windows (portable), Linux (`tar.gz` — chosen over AppImage because AppImage requires POSIX symlinks that Windows hosts cannot create), and macOS (`dir` → `.app`, only from a macOS host per `electron-builder` 26 policy). New npm scripts `package:linux`, `package:mac`, `package:all`. A GitHub Actions workflow (`.github/workflows/release.yml`) runs each platform on its native runner. A POSIX launcher (`abrir-painel.sh`) mirrors the Windows batch file; `scripts/package-portable.cjs` refuses to run outside Windows with a clear message.
- Verified the Linux build from Windows: `dist/APPDashboard-linux-x64.tar.gz` (123 MB, 88 entries, includes the ELF binary and locales).
